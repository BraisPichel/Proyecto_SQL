...
SELECT AVG(amount) AS "Promedio_pagado" ,  
	   STDDEV(amount) AS "Desviación_estándar_pagado" , 	
	   VARIANCE(amount) AS "Varianza_pagado"
FROM payment AS p ;

-- 27. ¿Qué películas se alquilan por encima del precio medio?

SELECT title "Título_pelicula",
...
ON fc.category_id = c.category_id AND c."name" = 'Music'
...
SELECT c.name AS "Categoría_película", 
       COUNT(f.film_id) AS "Total_peliículas"
FROM category c
